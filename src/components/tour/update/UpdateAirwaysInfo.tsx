...
interface UpdateAirwaysInfoProps {
  formik: FormikProps<TourUpdateType>;
  airways: AirwayType[];
  cities: CityType[];
}

const UpdateAirwaysInfo: React.FC<UpdateAirwaysInfoProps> = ({
  formik,
  airways,
  cities,
}) => {
          ...
          {({ push, remove }) => (
            <>
              {formik.values.airways.map((airway, index) => (
                <div className="row" key={"general-info-update" + index}>
                  <div className="col-md-4 mb-3">
                    <label
                      className="form-label"
                      htmlFor={`tour-airways-${index}`}
                    >
                      {t("airways")}
                    </label>
                    <select
                  ...
                  <div className="col-md-4 mb-3">
                    <label
                      className="form-label"
                      htmlFor={`tour-airways-city_from-${index}`}
                    >
                      {t("departure-from")}
                    </label>
                    <select
                      ...
                      value={airway.city_from?.id || 0}
                      onChange={(event) => {
                        const selectedValue = parseInt(event.target.value, 10);
                        formik.setFieldValue(
                          `airways[${index}].city_from_id`,
                          selectedValue,
                        );
                      }}
                      onBlur={formik.handleBlur}
                  ...
                  <div className="col-md-4 mb-3">
                    <label
                      className="form-label"
                      htmlFor={`tour-airways-city_to-${index}`}
                    >
                      {t("city-to")}
                    </label>
                    <select
                      ...
                      value={airway.city_to?.id || 0}
                      onChange={(event) => {
                        const selectedValue = parseInt(event.target.value, 10);
                        formik.setFieldValue(
                          `airways[${index}].city_to_id`,
                          selectedValue,
                        );
                      }}
                      onBlur={formik.handleBlur}
                  ...
                  <div className="col-md-4 mb-3">
                    <label
                      className="form-label"
                      htmlFor={`tour-airways-time_from-${index}`}
                    >
                      {t("time-from")}
                    </label>
                    <input
                      ...
                      value={
                        airway.from ? formatDateToInputValue(airway.from) : ""
                      }
                  ...
                  <div className="col-md-4 mb-3">
                    <label
                      className="form-label"
                      htmlFor={`tour-airways-time_to-${index}`}
                    >
                      {t("time-to")}
                    </label>
                    <input
                      ...
                      value={airway.to ? formatDateToInputValue(airway.to) : ""}
                      onChange={formik.handleChange}
                      onBlur={formik.handleBlur}
                    />
                  </div>
                  <div className="col-md-4 mb-3">
                    <label
                      className="form-label"
                      htmlFor={`tour-airways-position-${index}`}
                    >
...
export default UpdateAirwaysInfo;
