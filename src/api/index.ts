export * from "./admin/get";
export * from "./admin/auth";
export * from "./admin/getAll";
export * from "./admin/create";
export * from "./admin/update";
export * from "./admin/delete";
export * from "./admin/getById";
export * from "./admin/activate";
export * from "./admin/deactivate";

export * from "./airway/get";
export * from "./airway/getAll";
export * from "./airway/create";
export * from "./airway/update";
export * from "./airway/delete";

export * from "./application/get";
export * from "./application/getAll";
export * from "./application/delete";

export * from "./city/get";
export * from "./city/getAll";
export * from "./city/create";
export * from "./city/update";
export * from "./city/delete";

export * from "./extraPackage/get";
export * from "./extraPackage/getAll";
export * from "./extraPackage/create";
export * from "./extraPackage/update";
export * from "./extraPackage/delete";

export * from "./hotels/get";
export * from "./hotels/getAll";
export * from "./hotels/create";
export * from "./hotels/update";
export * from "./hotels/delete";

export * from "./organization/get";
export * from "./organization/getAll";
export * from "./organization/create";
export * from "./organization/update";
export * from "./organization/delete";

export * from "./setting/get";
export * from "./setting/getAll";
export * from "./setting/delete";

export * from "./tour/get";
export * from "./tour/getAll";
export * from "./tour/delete";

export * from "./news/get";
export * from "./news/getAll";
export * from "./news/create";
export * from "./news/update";
export * from "./news/delete";

export * from "./room/get";
export * from "./room/getAll";
export * from "./room/create";
export * from "./room/update";
export * from "./room/delete";

export * from "./role/items/list";
export * from "./role/items/create";
export * from "./role/items/getAll";
export * from "./role/getById";
export * from "./role/getAll";
export * from "./role/create";
export * from "./role/update";

export * from "./modules/getModule";
export * from "./modules/getModuleItem";

